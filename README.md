...
## 수동 실행 (Actions 탭 → "CGV 예매일 감시" → Run workflow)

| 옵션 | 하는 일 |
|---|---|
| (아무것도 안 켬) | 감시 잡을 하나 더 띄운다. 대기열에 들어가 현재 잡이 끝나면 이어받는다 |
| `test_alert` | 실제 알림과 똑같은 모양의 샘플 알림을 1건 보낸다 (감시 기준선은 건드리지 않음) |
| `diagnose` | **메시지를 보내지 않고** 봇/채팅 상태만 조회한다 (`getMe`, `getChat` 등) |

## 텔레그램 속도 제한(429) 주의

텔레그램은 제한이 걸린 상태에서 요청을 또 보내면 **대기 시간을 다시 연장**한다.
그래서 실패했다고 곧바로 재시도하면 영영 풀리지 않는다. 실제로 그렇게 막힌 적이 있다.

코드는 429 응답의 `retry_after` 를 읽어서 이렇게 처리한다.

- **120초 이내** → 그만큼 기다렸다 재시도
- **그보다 길면** → 이번 회차 포기. 기준선을 갱신하지 않으므로 **1분 뒤 루프가 자동 재시도**한다.
  알림이 유실되지 않고 몇 분 늦어질 뿐이다.

수동으로 테스트할 때는 **한 번 보내고 최소 15분은 두는 것**이 좋다.
